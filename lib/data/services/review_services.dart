...
class ReviewServices {
  ...
  Stream<List<EnhancedReviewModel>> getProductReviewsWithUserDetails(
      String productId) {
    return _firestore
        .collection('reviews')
      ...
      for (var doc in snapshot.docs) {
        ...
        if (userDoc.exists) {
          ...
          enhancedReviews.add(EnhancedReviewModel(
              review: reviewModel, userName: 'Anonymous', userImageUrl: null));
        }
      }
  ...
  Future<void> deleteReview(String reviewId) async {
    try {
      ...
      log('SERVICES: Successfully deleted review with ID: $reviewId');
    } catch (error) {
      log('SERVICES: Failed to delete review: $error');
      throw Exception('SERVICES: Failed to delete review: $error');
    }
  }
}
